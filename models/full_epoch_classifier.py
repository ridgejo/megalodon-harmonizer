...
def _make_full_epoch_classifier(
    dataset_sizes,
    use_data_block,
    subject_ids,
    use_sub_block,
...
class FullEpochClassifier(nn.Module):

    def __init__(self, dataset_layer, subject_block, channels, time, output_classes):

        super(FullEpochClassifier, self).__init__()
        ...
        self.lin1 = nn.Linear(
            in_features=channels * time,
            out_features=1 if output_classes == 2 else output_classes,
        )
    ...
    def forward(self, x, labels, dataset_id, subject_id):
        ...
        x = x.flatten(start_dim=1, end_dim=2) # [B, C, T] => [B, C * T]
