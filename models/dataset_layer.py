...
class DatasetLayer(nn.Module):
    ...
    def __init__(self, dataset_sizes : dict, shared_dim=128):
        """
        Args:
        ...
        self.dataset_encoders = {
            dataset_id : nn.Conv1d(in_channels=data_channels, out_channels=shared_dim, kernel_size=1) for dataset_id, data_channels in dataset_sizes.items()
        }

        self.dataset_decoders = {
            dataset_id : nn.Conv1d(in_channels=shared_dim, out_channels=data_channels, kernel_size=1) for dataset_id, data_channels in dataset_sizes.items()
        }
