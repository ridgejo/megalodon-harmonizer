        ...
        for t in range(x.size(0)):  # iterate over time steps
            for layer in range(self.lstm.num_layers):
                if t == 0:
                    ...
                    hx = all_hidden_states[-1][layer][0]
                    cx = all_hidden_states[-1][layer][1]
                
                # Input-to-hidden
                gates = torch.mm(x[t] if layer == 0 else all_hidden_states[-1][layer - 1][0], weight_ih_l[layer].t()) + bias_ih_l[layer]
                
                # Hidden-to-hidden
                gates += torch.mm(hx, weight_hh_l[layer].t()) + bias_hh_l[layer]
                ...
                if t == 0:
                    all_hidden_states.append([(hx, cx)])
                else:
                    all_hidden_states[-1].append((hx, cx))
